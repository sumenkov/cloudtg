...
- Загрузка, скачивание и открытие файлов (включая открытие папки в ОС)
- Перемещение и удаление файлов, удаление пустых папок
- Поделиться файлом (пересылка сообщения)
- Фоновый индексатор (обновления из канала без ручного импорта)
- Поиск файлов по имени/типу (в папке или по всему хранилищу)
- Реконсайл последних сообщений канала хранения (пометка битых файлов/папок)
- Восстановление битых файлов и папок
- Бэкап/восстановление базы в отдельном канале CloudTG Backups
- Импорт вручную добавленных файлов из канала хранения:
...
- **Реконсайл последних N сообщений** — проверяет последние сообщения в канале хранения
  и помечает битые записи. Если папка старая — увеличь лимит.
- **Бэкап базы** — отправляет файл SQLite в канал **CloudTG Backups**.
- **Восстановить** — берёт последний бэкап или пересобирает базу из канала хранения.
  Восстановление применяется после перезапуска приложения.
- **Открыть канал** — открывает канал **CloudTG Backups** в Telegram.

Если ты вышел из канала или потерял к нему доступ — воспользуйся кнопкой создания нового канала.

На главном экране есть кнопка **«Запустить импорт»** — она запускает ручной импорт
новых файлов из канала хранения.
Также при запуске приложения и далее в фоне новые сообщения из канала хранения
индексируются автоматически.

## Импорт из канала хранения
При запуске, в фоне и по кнопке «Запустить импорт» приложение просматривает новые сообщения
в канале хранения и добавляет вручную загруженные файлы:
- файлы без тегов попадают в папку «Неразобранное»
- если в подписи есть хештег папки, файл кладётся в эту папку (или папка создаётся)

## Поиск
В разделе файлов доступны фильтры по имени и типу. Поиск можно выполнять
в текущей папке или по всему хранилищу.

## Битые файлы и папки
Если сообщение в канале хранения удалено или недоступно, запись помечается как «битая»
(обычно после реконсайла).
