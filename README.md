...
> API_ID и API_HASH берутся из переменных окружения `CLOUDTG_API_ID` и `CLOUDTG_API_HASH`
...
5) Задай их через переменные окружения `CLOUDTG_API_ID` и `CLOUDTG_API_HASH`
   (можно создать файл `.env` на основе `.env.example`).
...
Ключи нужно передать при запуске (через env или `.env` рядом с приложением).
Пример:
```bash
