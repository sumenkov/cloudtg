...
Подсказка: если ключи сохранены в зашифрованном файле, CloudTG попросит пароль при каждом запуске.
...
- Если отправить файл напрямую в канал **CloudTG** (не через кнопку `Выбрать и загрузить`), CloudTG импортирует его при синхронизации и положит в папку указанную в теге (например `#книги`) или `Неразобранное` (если в подписи нет тега папки).
...
Обычно это значит “`tdjson.dll` нашли, но Windows не смог загрузить её зависимости”. Причины обычно такие:
- не та разрядность (x86 vs x64);
- рядом нет зависимых DLL (часто это `libssl-*.dll`, `libcrypto-*.dll`, `zlib1.dll`);
- в системе нет Microsoft Visual C++ Runtime (редко, но бывает).

Что сделать по-быстрому:
1. Убедись, что рядом с `CloudTG.exe` лежит не только `tdjson.dll`, но и остальные DLL из архива TDLib.
2. Либо укажи путь к `tdjson.dll` через кнопку `Открыть` (важно: зависимые DLL должны лежать рядом с `tdjson.dll`).
3. Если всё равно не заводится, приложи лог (см. раздел выше) и напиши в Issues.
...
- Метаданные структуры в базе данных (имена, дерево, `tg_chat_id`, `tg_msg_id`).
...
- `API_ID/API_HASH` не хранятся в базе данных.
